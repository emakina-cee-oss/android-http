...
public class ImageProcessor extends DataProcessor<Bitmap, Bitmap> {
     ...
     *
     * @param bitmapOptions custom {@link BitmapFactory.Options}
     */
    ...
    public void processWebReply(Context c, ReplyAdapter r, Handler handler) {
        Log.i(TAG, "processing imagerequest from: " + r.getRequest().getUrl());
    ...
    public Bitmap obtainDataObjectFromWebReply(Context c, ReplyAdapter reply) {
        ...
        try {
            ...
            Log.w(TAG, "Could not cache bitmap (sync webrequest)", tr);
        }
        return b;
    }
     ...
     * @param object the {@link CachedObject} obtained from the cache database
    ...
    public void processCachedObject(CachedObject cachedObject, Handler handler, Request request) {
        ...
        switch (cachedObject.getFrom()) {
            case MEMORY:
                Log.i(TAG, "processing image from memory cache");
                processMemCache(cachedObject.getCachedObject(), handler, wr);
                break;
            case FILE:
                Log.i(TAG, "processing image from file cache");
     ...
     *
     * @param url     the url of the image file (web url)
    ...
    public static class ImageProcessHandler extends Handler {
         ...
         * Constructor
         *
         * @param imageView       the {@link ImageView} the image should be displayed on
         * @param url             the of the image, will be set as {@link ImageView} tag in
         *                        order to identify the correct {@link ImageView}
         * @param fadeInAnimation an optional {@link Animation}
         * @param defaultImage    a default image to be displayed if the real image cannot
         *                        be displayed
         */
         ...
         *                        order to identify the correct {@link ImageView}
         * @param fadeInAnimation an optional {@link Animation}
         * @param defaultImage    a default image to display if the image returned by the
         ...
         * present.
         *
         * @param bitmap the {@link Bitmap} to be processed
        ...
        public void handleMessage(Message msg) {
            if (msg.arg1 == ServiceProcessor.RETURN_MESSAGE_OK) {
                ...
                Log.i(TAG, "adding image from url:" + url + " for tag: " + imageView.getTag());
