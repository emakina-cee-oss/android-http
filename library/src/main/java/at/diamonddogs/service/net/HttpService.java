...
public class HttpService extends Service implements WebClientReplyListener {
     ...
     *
     * @param handler    the handler that will be informed once the {@link WebRequest}
     *                   has been completed
     * @param webRequest the {@link WebRequest} to run
     ...
     *                         has been completed
     * @param webRequest       the {@link WebRequest} to run
     * @param progressListener a {@link ProgressListener} that will be informed of download
     *                         progress
     * @return a {@link WebRequestReturnContainer}
     */
    public WebRequestReturnContainer runWebRequest(final Handler handler, final WebRequest webRequest,
        ...
        if (processor == null) {
            ...
            if (id == -1) {
                throw new ServiceException("processor id == -1 looks like you forgot to set a Processor ID for the " +
                        "WebRequest");
            }
            throw new ServiceException("No processor with id '" + id + "' has been registered! WebRequest originally " +
                    "created: ",
     ...
     *
     * @param webRequests the {@link WebRequest}s to run
     ...
     *
     * @param webRequests      the {@link WebRequest}s to run
     * @param progressListener a single {@link DownloadProgressListener} that will receive
     ...
     * @return an array of {@link WebRequestReturnContainer}s
     */
    public WebRequestReturnContainer[] runSynchronousWebRequests(WebRequest[] webRequests, DownloadProgressListener
            progressListener) {
     ...
     *
     * @param webRequests       the {@link WebRequest}s to run
     * @param progressListeners an array of {@link DownloadProgressListener} that will receive
     ...
     * wrapped in a {@link WebRequestReturnContainer}.
     */
    public WebRequestReturnContainer runSynchronousWebRequest(final WebRequest webRequest, final
    DownloadProgressListener progressListener) {
        SynchronousProcessor<?> synchronousProcessor = (SynchronousProcessor<?>) registeredProcessors.get(webRequest
                .getProcessorId());
        WebRequestReturnContainer ret = new WebRequestReturnContainer();
        ret.id = webRequest.getId();
        ret.successful = true;
        try {
            CachedObject cachedObject = getCachedObject(webRequest);
     ...
     *
     * @param webRequests      the {@link WebRequest}s to run
     * @param progressListener the {@link DownloadProgressListener} that will receive updates
     ...
     *
     * @param webRequests      the {@link WebRequest}s to run
     * @param progressListener the {@link DownloadProgressListener}s that will receive
    ...
    private Future<ReplyAdapter>[] runSynchronousWebRequestsFuture(WebRequest[] webRequests,
                                                                   DownloadProgressListener[] progressListeners) {
     ...
     *
     * @param webRequest       the {@link WebRequest} to run
     * @param progressListener a {@link DownloadProgressListener}, can be <code>null</code>
     * @return a {@link Future} that allows the caller to wait for the result
     */
    private Future<ReplyAdapter> runSynchronousWebRequestFuture(final WebRequest webRequest, final
    DownloadProgressListener progressListener) {
        ...
        if (processor instanceof SynchronousProcessor<?>) {
            ...
            throw new ServiceException("Invalid ServiceProcessor for sync WebRequest, only instances of " +
                    "SynchronousProcessor are allowed");
        }
        return null;
    }
     ...
     *
     * @param processor an instance of the {@link ServiceProcessor} to register
     */
    public void registerProcessor(ServiceProcessor<?> processor) {
        ...
        if (isProcessorRegistered(processorId)) {
            if (registeredProcessors.get(processorId).getClass() != processor.getClass()) {
                ...
                throw new ProcessorExeception("A processor known by id " + processorId + " has already been " +
                        "registered.");
            }
        }
     ...
     *
     * @param id the id of the {@link ServiceProcessor} to unregister
     */
     ...
     * {@link HttpService}
     *
     * @param id the id of the {@link ServiceProcessor}
     ...
     *
     * @param id the id of the {@link ServiceProcessor} to obtain
    ...
    private void dispatchWebReplyProcessor(ReplyAdapter replyAdaper, Handler handler) {
        try {
            if (!workerQueue.isShutDown()) {
                getProcessor(replyAdaper.getRequest()).processWebReply(this.getApplicationContext(), replyAdaper,
                        handler);
            } else {
     ...
     * @param serviceConnection a {@link ServiceConnection}
     */
     ...
     *
     * @param id the id of the {@link WebRequest} to cancel
     */
    ...
    private Future<ReplyAdapter> getWebRequestTask(WebRequest webRequest, DownloadProgressListener
            downloadProgressListener, boolean async) {
    ...
    private CachedObject getCachedObject(WebRequest webRequest) {
        if (webRequest.getCacheTime() < 0) {
            return null;
        }
        CacheManager cm = CacheManager.getInstance();
        return cm.getFromCache(HttpService.this, webRequest);
    }
